...
import { parseTeachersExcel, TeacherData } from "../../lib/excel";
...
export default function TeachersPage() {
  ...
  const moveSection = (spec: string, direction: 'up' | 'down') => {
    ...
    console.log(`Attempting to move ${spec} ${direction}`);
    const currentIndexVisible = visibleSpecs.indexOf(spec);
    if (currentIndexVisible === -1) {
        console.warn(`Spec ${spec} not found in visibleSpecs`, visibleSpecs);
        return;
    }

    const targetIndexVisible = direction === 'up' ? currentIndexVisible - 1 : currentIndexVisible + 1;
    if (targetIndexVisible < 0 || targetIndexVisible >= visibleSpecs.length) {
        console.warn(`Target index ${targetIndexVisible} out of bounds for visibleSpecs length ${visibleSpecs.length}`);
        return;
    }
    
    const targetSpec = visibleSpecs[targetIndexVisible];
    console.log(`Targeting swap with ${targetSpec}`);
    ...
    if (currentIndexFull === -1 || targetIndexFull === -1) {
        console.error(`Indices not found in full list: ${currentIndexFull}, ${targetIndexFull}`);
        return;
    }
