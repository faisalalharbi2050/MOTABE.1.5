...
const SupervisionManageSchedulesModal: React.FC<Props> = ({
  isOpen, onClose, supervisionData, setSupervisionData, showToast
}) => {
  ...
  const handleSaveCurrent = () => {
    ...
    const newSchedule: SavedSupervisionSchedule = {
      id: Date.now().toString(),
      name: newScheduleName.trim(),
      createdAt: new Date().toISOString(),
      dayAssignments: [...supervisionData.dayAssignments],
      isApproved: supervisionData.isApproved,
    };
              ...
              <div className="bg-white rounded-[2rem] p-5 shadow-sm border border-slate-100 h-full flex flex-col">
                ...
                {supervisionData.dayAssignments.length === 0 ? (
                  ...
                  <div className="space-y-4 flex-1">
                    <div className="bg-[#e5e1fe]/30 p-3 rounded-xl border border-[#e5e1fe] flex items-center justify-between">
                       <span className="text-sm font-bold text-[#655ac1]">حالة الجدول:</span>
                       <span className={`text-xs font-bold px-2 py-1 rounded-md ${supervisionData.isApproved ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'}`}>
                         {supervisionData.isApproved ? 'معتمد' : 'غير معتمد'}
                       </span>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full border border-slate-200 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-[#655ac1] focus:ring-1 focus:ring-[#655ac1]"
                      />
                    </div>
                  </div>
                )}
                ...
                <button
                  ...
                  disabled={supervisionData.dayAssignments.length === 0 || savedSchedules.length >= 10 || !newScheduleName.trim()}
                ...
                {savedSchedules.length === 0 ? (
                  ...
                  <div className="space-y-3 max-h-[450px] overflow-y-auto pr-2 custom-scrollbar">
                    {savedSchedules.map(schedule => (
                      <div key={schedule.id} className={`p-4 rounded-xl border-2 transition-all ${supervisionData.activeScheduleId === schedule.id ? 'border-[#655ac1] bg-[#655ac1]/5 shadow-sm' : 'border-slate-100 bg-white hover:border-[#655ac1]/40'}`}>
                         <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div className="flex-1">
                               {editingId === schedule.id ? (
                                 <div className="flex items-center gap-2">
                                   <input
                                      ...
                                      className="border border-[#655ac1] rounded-lg px-2 py-1 text-sm font-bold text-slate-800 outline-none"
                                   />
                               ...
                               {supervisionData.activeScheduleId === schedule.id && !schedule.isApproved && (
                                 <Badge variant="warning" className="mt-2 text-[10px] py-0">الجدول المعروض الأن</Badge>
                               )}
                            </div>

                            <div className="flex flex-wrap items-center gap-2 justify-end">
                               {/* Only show Load if it's not currently the active one OR if there are unsaved UI changes in builder? 
                                   Just always show it. */}
                               {supervisionData.activeScheduleId !== schedule.id && (
                                 <button
