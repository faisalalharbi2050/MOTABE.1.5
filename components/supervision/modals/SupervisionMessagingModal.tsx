...
import { SchoolInfo, SupervisionScheduleData, Teacher, Admin, SupervisionMessage } from '../../../types';
  ...
  const messages = useMemo(() => {
    ...
    daysToProcess.forEach(day => {
      ...
      da.staffAssignments.forEach(sa => {
        ...
        // Explicit fallback: If sa.staffType is somehow missing in older saved data, we check if the ID exists in the 'teachers' array.
        const staffType = sa.staffType || (teachers.some(t => t.id === sa.staffId) ? 'teacher' : 'admin');

        const baseContent = messageType === 'assignment'
          ? generateAssignmentMessage(sa.staffName, staffType, day, locationNames, supervisionData.effectiveDate, schoolInfo.gender)
          : generateReminderMessage(sa.staffName, staffType, day, locationNames, schoolInfo.gender);
                 ...
                 <div key={msg.id} className="bg-slate-50 rounded-2xl p-5 border border-slate-100 hover:border-[#655ac1]/20 transition-all shadow-sm hover:shadow-md group">
                   <div className="flex items-center justify-between mb-4">
                     <div className="flex items-center gap-3">
                       ...
                       <span className="text-xs font-bold px-2 py-1 rounded bg-indigo-100 text-indigo-700 border border-indigo-200">{DAY_NAMES[msg.day]}</span>
                     </div>
                     <div className="flex gap-2">
                       <button
                         onClick={() => {
